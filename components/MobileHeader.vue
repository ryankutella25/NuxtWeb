    ...
    <div v-if="isOpen" class="fixed inset-0 z-50 md:hidden">
      ...
      <aside class="relative ml-auto flex h-full w-[84vw] max-w-80 flex-col overflow-y-auto border-l border-white/10 bg-slate-950 p-5">
        ...
        <nav class="flex flex-col gap-2">
          <NuxtLink to="/" :class="['border-l-2 px-3 py-2 text-sm font-medium transition', mobileNavLinkClass('/')]" @click="isOpen = false">Home</NuxtLink>
          <NuxtLink to="/projects" :class="['border-l-2 px-3 py-2 text-sm font-medium transition', mobileNavLinkClass('/projects')]" @click="isOpen = false">Projects</NuxtLink>
          <NuxtLink to="/aboutme" :class="['border-l-2 px-3 py-2 text-sm font-medium transition', mobileNavLinkClass('/aboutme')]" @click="isOpen = false">About</NuxtLink>
        </nav>

        <div class="mt-auto flex flex-col gap-2 pt-6">
          <a href="https://github.com/ryankutella25" target="_blank" rel="noreferrer" aria-label="GitHub" class="grid grid-cols-[1.35rem_1fr] items-center gap-2 rounded-md px-3 py-2 text-base font-medium text-slate-300 transition hover:bg-white/10">
          ...
          <a href="https://www.linkedin.com/in/ryankutella/" target="_blank" rel="noreferrer" aria-label="LinkedIn" class="grid grid-cols-[1.35rem_1fr] items-center gap-2 rounded-md px-3 py-2 text-base font-medium text-slate-300 transition hover:bg-white/10">
            <svg class="h-[1.35rem] w-[1.35rem]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          ...
          <a href="mailto:ryankutella25@yahoo.com" class="grid grid-cols-[1.35rem_1fr] items-center gap-2 rounded-md px-3 py-2 text-base font-medium text-slate-300 transition hover:bg-white/10">
